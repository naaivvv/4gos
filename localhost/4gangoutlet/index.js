            ...
            fetch("fetch_latest_states.php")
                ...
                .then(data => {
                    if (JSON.stringify(lastStates) !== JSON.stringify(data)) {
                        lastStates = data;
                        document.querySelector("#socket-3").checked = data.s1;
                        document.querySelector("#socket-2").checked = data.s2;
                        document.querySelector("#socket-1").checked = data.s3;
                        document.querySelector("#socket-0").checked = data.s4;
        
                        // Send state once to the /control endpoint
                        for (let pin = 0; pin < 4; pin++) {
                            sendState(pin, data[`s${3 - pin}`]);
                        }
                    }
                })
